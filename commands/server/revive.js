...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(option => option.setName('question').setDescription('the question you wish to revive the server with').setRequired(false)),

    async execute(interaction) {
        var check = await checkPerms(interaction, '737309369741606983', '878371753934585856')
        if (check) {
            ...
            var RandQuest = interaction.options.getString('question') || randomquestionArr[Math.floor(Math.random() * randomquestionArr.length)];
